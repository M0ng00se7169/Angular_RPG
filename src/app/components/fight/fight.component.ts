...
export class FightComponent {
            ...
            && this.currentCharacter.level > 2) {
                ...
                if (this.currentCharacter.turnsUntilSpecialAvailableAgain) {
                    ...
                    if (this.currentCharacter instanceof Warrior) {
                        this.availableTargets = Teams.enemies;
                        this.displayMessage = "Attack two targets at once with a small attack penalty. " +
                        "At level 6 and above, the attack penalty is removed. The two targets may be the same enemy."
                    }
                    if (this.currentCharacter instanceof Ranger) {
                        this.availableTargets = Teams.heroes;
                        this.displayMessage = "Setup a trap to protect one of your heroes, " +
                            "the trap will prevent all damage and the enemy will take a turn to free itself from the trap. " +
                            "At level 6 and above, the trap will also deal up to 8 damage."
                    }
                    if (this.currentCharacter instanceof Rogue) {
                        this.availableTargets = Teams.enemies;
                        this.displayMessage = "Poison an enemy or add another stack of poison to an enemy to do up to 3 damage, " + 
                            "with each stack of poison multiplying the damage. At level 6 and above, " + 
                            "the damage is up to 6 times the number of poison stacks."
                    }
                    if (this.currentCharacter instanceof Priest) {
                        this.availableTargets = Teams.heroes;
                        this.displayMessage = "Select a hero to heal for up to 6 health plus an additional " +
                            "point for each point in the intelligence skill. At level 6 and above, you choose two targets two heal. " +
                            "The two targets can be the same hero."
                    }
                }
        }
    }
    ...
    rangerSpecialAttack(target: BaseCharacter, upgraded: boolean) {
        ...
        this.displayMessage = `${this.currentCharacter.name} set up to protect ${target.name}`;
    ...
    priestSpecialAttack(target: BaseCharacter, upgraded: boolean) {
        ...
        if (upgraded) {
            ...
            this.displayMessage = `${target.name} was healed for ${healing} healing.`;
            setTimeout(() => {
                this.nextTurn();
            }, this.actionDelay);
        }
    ...
    takeEnemyTurn() {
        if (this.currentCharacter instanceof Monster && this.currentCharacter.isTrapped) {
            ...
            while (!target) {
                ...
                let potencialTarget = this.heroParty[randomTargetIndex];
                if (!potencialTarget.isIncapacitated) {
                    target = potencialTarget;
                }
            }
